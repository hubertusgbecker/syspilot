...
.. story:: Custom Agent Workflow Tailoring
   ...
   **Acceptance Criteria:**
   ...
   4. Given a ``syspilot.<name>.tailoring.md`` file colocated with its agent in ``.github/agents/``, When the user opens the agent picker in VS Code, Then the tailoring file does not appear as a selectable or invocable agent — its frontmatter declares ``user-invocable: false`` and ``disable-model-invocation: true``
